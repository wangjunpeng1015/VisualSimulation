<template>
  <div class="header">
    ...
    <div class="userInfo layout-row">
      <p v-if="isLogin" id="longlat">经纬度：<span></span></p>
      <Avatar class="icon" icon="person"/><!--用户图片列子 src="https://i.loli.net/2017/08/21/599a521472424.jpg" -->
      <Dropdown v-if="isLogin" class="user" @on-click="dropmenu" placement="bottom-start">
        <a href="javascript:void(0)">
          {{username}}
          <Icon type="arrow-down-b"></Icon>
        </a>
        <DropdownMenu slot="list">
          <DropdownItem name="usermanage">XXXX</DropdownItem>
          <DropdownItem name="usermanage">XXXX</DropdownItem>
          <DropdownItem v-if="authority == 0" name="usermanage">用户管理</DropdownItem>
        </DropdownMenu>
      </Dropdown>
      <span>{{time}}</span>
...
export default {
  ...
  computed:{
    ...
    ...mapState(['username','isLogin','authority']),
  },
  props:['title'],
  mounted(){
    getTime = setInterval(()=>{
